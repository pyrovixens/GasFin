  ...
  return (
    <>
      ...
      >
        ...
        {/* Banking Security Session Indicator */}
        {(!isSidebarCollapsed || window.innerWidth < 768) && (
          <div className="px-3 pb-2">
            <div className="w-full p-2.5 rounded-xl border border-slate-800 bg-slate-900/60 text-xs font-bold flex items-center justify-between text-slate-300 shadow-sm">
              <div className="flex items-center gap-2 truncate">
                <ShieldCheck size={16} className="text-emerald-400 flex-shrink-0" />
                <span className="truncate text-[11px] text-slate-300 font-semibold">{supabaseUser?.email || 'Sesión Bancaria Activa'}</span>
              </div>
              <span className="text-[9px] uppercase font-black px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex-shrink-0">
                Segura
              </span>
            </div>
          </div>
